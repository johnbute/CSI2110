...
public class AlgAnalysis {
	...
	public static void arraySortRuntime(int count, int maxSize){
		...
		for(int i = 1; i<= count; i++){
			...
			double ratio1 = (double) minTime / (i*i);
			double ratio2 = (double) minTime/(i * Math.log(size) / Math.log(2));
			System.out.print("N: " + size + ", T(n): " + minTime + ", T(n)/(n*n): " +  ratio1 + ", T(n)/(nlogn): " + ratio2 + "\n");
		}
	...
	public static long unique1Runtime(int n){
		int[] genArray = new int [n];
		genArray = genArray(n);
		genArray = randomizeArray(genArray);
		long startTime = System.nanoTime();
		Unique1.unique1(genArray);
		long endTime = System.nanoTime();
