...
public class SequenceAlgs<E> {
	...
	public boolean isPalindrome (Sequence<E> S) {
    	...
    	for (int i = 0; i < size / 2; i++) {
        	if (!S.get(i).equals(S.get(size - 1 - i))) {
            return false;
        }
    }
    return true;
	...
	public static <E> void swap(Sequence<E> S, Position<E> x, Position<E> y) {
		E temp = x.getElement();
	...
	public void inplaceReverse(Sequence<E> S) {
		...
		int size = S.size();
    	for (int i = 0; i < size / 2; i++) {
        	Position<E> x = S.positionAtIndex(i);
        	Position<E> y = S.positionAtIndex(size - 1 - i);
			swap(S, x, y);
    	}
	}
